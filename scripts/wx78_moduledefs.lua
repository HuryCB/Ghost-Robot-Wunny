...
local function movespeed_activate(inst, wx)
    ...
    print("caiu no aumento do wx78")
    wx.components.locomotor.runspeed = TUNING.WILSON_RUN_SPEED * (2 + TUNING.WX78_MOVESPEED_CHIPBOOSTS[wx._movespeed_chips + 1])
    wx.components.locomotor:SetExternalSpeedMultiplier(wx,"moveSpeedChipBoost", TUNING.WX78_MOVESPEED_CHIPBOOSTS[wx._movespeed_chips + 1])
end

local function movespeed_deactivate(inst, wx)
    wx._movespeed_chips = math.max(0, wx._movespeed_chips - 1)
    print("caiu na diminuicao do wx78")
