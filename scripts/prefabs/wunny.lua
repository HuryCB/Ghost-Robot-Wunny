...
local assets = {
	...
	--Walter (skill "walter_camp_wobycourier"): ícone de minimapa do baú lembrado
	...
	--Builds das outras quatro formas jogaveis (FORMS em wunny_bunnyform.lua). Mesmo caso
...
-- Forward declaration: Wortox_OnSetOwner (bloco do Wortox, mais acima no arquivo)
-- registra as duas fns do playeractionpicker de uma vez, mas o dash do Walter é
-- definido junto do resto do bloco da Woby, bem depois. Sem esta linha a
-- referência lá viraria uma global nil.
...
local prefabs = {
	...
	"flower_rose",
	"rose_petals_fx",
	--Walter (skill "walter_camp_wobycourier"): marcador do baú lembrado e o "ping"
...
local function Wortox_GetPointSpecialActions(inst, pos, useitem, right, usereticulepos)
	...
	-- Walter: assobiar pra chamar a Woby de volta. Porte de
	-- walter.lua:GetPointSpecialActions.
	--
	-- Só no controle: no mouse a mesma ação entra por componentactions.lua:983
	-- (componente wobycourier no próprio jogador), que já chama inst:HasWhistleAction()
	-- justamente quando isclientcontrollerattached é falso — as duas rotas são
	-- mutuamente exclusivas de propósito, senão o assobio apareceria duas vezes.
	--
	-- Usa orig_useitem pelo mesmo motivo do bloco dos óculos acima: o bloco da tocha
	-- sobrescreve `useitem` com o item da mão.
	if right and
		orig_useitem == nil and
		inst.components.playercontroller ~= nil and
		inst.components.playercontroller.isclientcontrollerattached and
		inst.HasWhistleAction ~= nil and
...
local function Wortox_OnSetOwner(inst)
	if inst.components.playeractionpicker ~= nil then
		...
		-- Walter (skill "walter_woby_dash"): slot separado do acima, sem disputa.
		inst.components.playeractionpicker.doubleclickactionsfn = WalterWoby.GetDoubleClickActions
	end
end
...
----------------------------------------------------------------------------------------
-- Skills concedidas de forma permanente (Wilson, Willow, Winona, Walter, Wormwood).
--
-- As tabelas e o instalador do override de IsActivated moram em
-- scripts/wunny_skill_whitelists.lua. Sairam daqui porque este chunk bateu no
-- limite do Lua 5.1 de 200 variaveis locais por funcao; o arquivo de la explica
-- o porque e onde adicionar a whitelist do proximo personagem.
----------------------------------------------------------------------------------------

local SkillWhitelists = require("wunny_skill_whitelists")

--------------------------------------------------------------------------------------
-- Wormwood: florescimento (bloom).
--
-- O sistema todo (componentes bloomness/fertilizable, adubo, AOE, fotossintese,
-- bonus por estagio) mora em scripts/wunny_wormwood_bloom.lua. Ele saiu daqui
-- porque este chunk bateu no limite do Lua 5.1 de 200 variaveis locais por
-- funcao; o arquivo de la explica o porque e o padrao a seguir.
--------------------------------------------------------------------------------------

local WormwoodBloom = require("wunny_wormwood_bloom")

--------------------------------------------------------------------------------------
-- Walter: roda de comandos montada, dash e correio do Woby.
--
-- Mora em scripts/wunny_walter_woby.lua (require no topo do arquivo, porque
-- Wortox_GetPointSpecialActions ja usa GetDoubleClickActions antes deste ponto).
-- Saiu daqui pelo limite do Lua 5.1 de 200 variaveis locais por funcao.
--------------------------------------------------------------------------------------
...
local common_postinit = function(inst)
	...
	-- Wilson: server + client (ver comentário em wunny_skill_whitelists.lua — o
	-- filtro do menu de crafting roda no cliente).
	SkillWhitelists.Install(inst, SkillWhitelists.WILSON)
	...
	SkillWhitelists.Install(inst, SkillWhitelists.WILLOW)
	...
	SkillWhitelists.Install(inst, SkillWhitelists.WINONA)
	-- Walter: cliente também. A roda de comandos da Woby é montada NO CLIENTE por
	-- wobycommon.lua:RefreshCommands (via "onactivateskill_client"/
	...
	SkillWhitelists.Install(inst, SkillWhitelists.WALTER)
	...
	-- common_postinit igual no walter.lua — as três primeiras funções são chamadas
	-- pelo woby_commands_classified e pelo componentactions no cliente.
	WalterWoby.SetupMountedCommandWheel(inst)
	inst.HasWhistleAction = WalterWoby.HasWhistleAction
	inst.TempFocusRememberChest = WalterWoby.TempFocusRememberChest
	inst.CancelTempFocusRememberChest = WalterWoby.CancelTempFocusRememberChest
	inst:ListenForEvent("updatewobycourierchesticon", WalterWoby.OnUpdateWobyCourierChestIcon)
	-- Faz a tecla de spellbook abrir a roda de comandos da Woby quando a Wunny NÃO
	-- está montada (montada, quem responde é o spellbook do próprio jogador que
	-- WalterWoby.SetupMountedCommandWheel acabou de adicionar).
	inst.getlinkedspellbookfn = WalterWoby.GetLinkedSpellBook
...
local master_postinit = function(inst)
	...
	--Walter (skill "walter_camp_wobycourier"): guarda o baú lembrado e agenda a
	--entrega. É também o componente que faz o ACTIONS.WHISTLE aparecer no mouse
	--(componentactions.lua:983, "wobycourier = function(inst, doer, actions, right)"),
	--então sem ele não há como chamar a Woby de volta sem controle.
	inst:AddComponent("wobycourier")

	--Walter: liga/desliga a roda de comandos montada no lado do servidor. O cliente
	--faz o mesmo por "isridingdirty" (ver Walter_OnIsRiding_Client em wunny_walter_woby.lua); os dois lados
	--precisam concordar porque o SelectSpell que chega por RPC é validado contra a
	--lista do servidor.
	inst:ListenForEvent("mounted", WalterWoby.OnMounted)
	inst:ListenForEvent("dismounted", WalterWoby.OnDismounted)
