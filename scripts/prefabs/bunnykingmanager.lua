local BunnyKingManager = Class(function(self, inst)
    self.inst = inst
...
local function OnKingDeath(inst, data)
	local manager = TheWorld.components.bunnykingmanager
...
local function OnKingRemoval(inst, data)
	...
	TheWorld:PushEvent("onbunnykingdestroyed", {throne = manager.main_throne})
...
function BunnyKingManager:OnThroneDestroyed(throne)
...
function BunnyKingManager:CreateMermKing(candidate, throne)

	candidate.components.inventory:DropEverything()
    self.inst:RemoveEventCallback("onremove", OnCandidateRemoved, candidate)
	self.inst:RemoveEventCallback("death", OnCandidateRemoved, candidate)

	self.king = ReplacePrefab(candidate, "bunnyking")
    ...
    TheWorld:PushEvent("onbunnykingcreated", {king = self.king, throne = self:GetMainThrone()})
end

local MERMCANDIDATE_MUST_TAGS = {"merm"}
local MERMCANDIDATE_CANT_TAGS = {"player", "bunnyking", "mermguard"}
function BunnyKingManager:FindMermCandidate(throne)
...
function BunnyKingManager:ShouldGoToThrone(merm, throne)
...
function BunnyKingManager:IsCandidateAtThrone(candidate, throne)
	return throne and candidate and throne:IsNear(candidate, 0.5)
end

function BunnyKingManager:ShouldTransform(merm)
...
function BunnyKingManager:IsThroneValid(throne)
    return throne ~= nil
        and throne:IsValid()
...
function BunnyKingManager:GetKing()
	return self.king
end

function BunnyKingManager:GetCandidate(throne)
	return self.candidates[throne]
end

function BunnyKingManager:IsThroneCandidate(merm, throne)
	return self.candidates[throne] == merm
end

function BunnyKingManager:IsCandidate(merm)
	for k,v in pairs(self.candidates) do
		if v == merm then
			return true
		end
...
function BunnyKingManager:GetThrone(merm)
	for k,v in pairs(self.candidates) do
		if v == merm then
			return k
		end
...
function BunnyKingManager:GetMainThrone()
	return self.main_throne
end

function BunnyKingManager:IsThrone(throne)
...
function BunnyKingManager:LoadPostPass(newents, savedata)
