...
local assets =
{
    Asset("ANIM", "anim/cinnabunnfarm.zip"),
	Asset("ATLAS", "images/inventoryimages/cinnabunnfarm.xml"),
}
...
-- Constants
local CINNABUNNFARM_MAX_HARVESTS = 3 
CINNABUNNFARM_FULL_GROW_TIME = 480 * 4 -- 4 days

local levels =
{
    { amount=4, grow="carrot_4", idle="carrot_4_idle", hit="hit_carrot_4" }, 
    { amount=3, grow="carrot_3", idle="carrot_3_idle", hit="hit_carrot_3" },
    { amount=2, grow="carrot_2", idle="carrot_2_idle", hit="hit_carrot_2" },
    { amount=1, grow="carrot_1", idle="carrot_1_idle", hit="hit_carrot_1" },
    --{ amount=0, grow="carrot_0", idle="carrot_0_idle", hit="hit_carrot_0" },
...
local function StartGrowing(inst, giver, product)
    if inst.components.harvestable ~= nil then
        --local is_seed = product.prefab == "carrot_seeds"
        local max_produce = levels[1].amount
        local productname = "carrot"

        inst.components.harvestable:SetProduct(productname, max_produce)
        inst.components.harvestable:SetGrowTime(CINNABUNNFARM_FULL_GROW_TIME / max_produce)
...
local function accepttest(inst, item)
    ...
    elseif inst.remainingharvests == 0 then
        ...
        return false, "CINNABUNNFARM_NEEDSGUANO"
    elseif not (item.prefab == "carrot" or item.prefab == "carrot_seeds") then
        return false
    end
    return true
end
...
local function onacceptitem(inst, giver, item)
    if inst.remainingharvests == 0 then
        inst.remainingharvests = CINNABUNNFARM_MAX_HARVESTS
...
local function fn()
    ...
    inst.MiniMapEntity:SetIcon("cinnabunnfarm.tex")

    inst.AnimState:SetBank("cinnabunnfarm")
    inst.AnimState:SetBuild("cinnabunnfarm")
...
return Prefab("cinnabunnfarm", fn, assets, prefabs),
    MakePlacer("cinnabunnfarm_placer", "cinnabunnfarm", "cinnabunnfarm", "idle")
